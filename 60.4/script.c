...
static void timespec_diff(struct timespec *to1, struct timespec *to0, struct timespec *to_diff)
{
    long nanosec_diff;
    long million = 1e6;

    nanosec_diff = (to1->tv_sec * million + to1->tv_nsec / 1e3) - (to0->tv_sec * million + to0->tv_nsec / 1e3);
    to_diff->tv_sec = nanosec_diff / million;
    to_diff->tv_nsec = nanosec_diff % million;
}
...
int main(int argc, char *argv[])
{
    ...
    for (;;) {
        ...
        if (FD_ISSET(master_fd, &in_fds)) {
            struct timespec to_new, to_diff;
            char timestamp_str[40];
            size_t timestamp_str_len;
            num_read = read(master_fd, buf, BUF_SIZE);
            if (num_read <= 0) {
                exit(EXIT_SUCCESS);
            }

            clock_gettime(CLOCK_REALTIME, &to_new);
            timespec_diff(&to_new, &to, &to_diff);
            sprintf(timestamp_str, "%ld.%ld %ld\n", to_diff.tv_sec, to_diff.tv_nsec, num_read);
            to.tv_sec = to_new.tv_sec;
            to.tv_nsec = to_new.tv_nsec;
            ...
            if (write(script_timed_fd, timestamp_str, timestamp_str_len) != timestamp_str_len) {
                fprintf(stderr, "partial/failed write\n");
                exit(1);
            }
